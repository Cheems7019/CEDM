...
framework on a hub causal structure: Y1->(Y2, Y3, Y4, Y5, Y6).

Frameworks compared:
  - Conditional: 6 groups (G0: X0-X4 ... G5: X25-X29), Y2-Y6 each condition
    only on Y1 (the hub).
  - Standard: single joint model over all 30 features.

Data: TrueSampler_hub (6 slates x 5 nodes, hub structure).
...
from utils.utils_data import TrueSampler_hub, compute_conditional_expectation_hub
...
def main():
    parser = argparse.ArgumentParser(description="Hub structure: Conditional framework vs Standard framework comparison")
    ...
    os.makedirs("./results/hub_comparison", exist_ok=True)
    ...
    # Hub adjacency: G0 is the root; G1-G5 are leaf children.
    ...
    print("  Conditioning: Y2-Y6 all condition ONLY on Y1 (hub)")
    ...
    results_file = f"./results/hub_comparison/hub_comparison_results{scaler_suffix}.csv"
    ...
    for seed in range(n_seeds):
        ...
        for m in range(M):
            ...
            true_result = compute_conditional_expectation_hub(
                Y1_raw[m],
                sigma=sigma,
    ...
    overall_summary.to_csv(f"./results/hub_comparison/overall_summary{scaler_suffix}.csv")

    print(f"\nFinal results saved to ./results/hub_comparison/:")
    print(f"- Detailed results: hub_comparison_results{scaler_suffix}.csv")
