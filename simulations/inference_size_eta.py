"""
inference_size_eta.py

Conditional Diffusion Framework empirical-size analysis testing Y3 conditionally
independent of Y2 given Y1.

Data: TrueSampler_inference_size with gamma=1, eta=0 (no Y2->Y3 connection).
Model structure (fork, correctly specified under H0):
  - G0 (Y1 / X0-X9):   unconditional
  - G1 (Y2 / X10-X19): conditional on G0
  - G2 (Y3 / X20-X29): conditional on G0 only (fork)
...
Under H0 (eta=0), p-values should be approximately uniform; rejection rate at
...
def main():
    parser = argparse.ArgumentParser(description="Conditional Diffusion Framework-based conditional independence test with varying sample sizes (eta=0)")
    ...
    sigma = 1.0
    correlation = 0.5
    gamma = 1.0  # Y1->Y3 connection strength; held fixed (testing Y2->Y3)
    eta = 0.0    # no Y2->Y3 connection (H0)
    ...
    os.makedirs("./ckpt/inference_size_eta", exist_ok=True)
    os.makedirs("./results", exist_ok=True)
    os.makedirs("./results/inference_size_eta", exist_ok=True)
    os.makedirs("./data/inference_size_eta", exist_ok=True)
    ...
    # Fork structure: G0->G1, G0->G2, no G1->G2 edge (correct under eta=0).
    A = np.array([
        [0, 1, 1],  # G0->G1, G0->G2
        [0, 0, 0],  # G1 has no children
        [0, 0, 0],  # G2 has no children
    ], dtype=np.float32)

    print("\nConditional Diffusion Framework configuration for MCODEC test (η=0):")
    print("  Groups:")
    ...
    results_file = f"./results/inference_size_eta/inference_size_eta_results{scaler_suffix}.csv"
    ...
    for n_samples_train in training_samples_list:
        ...
        for seed in range(n_seeds):
            ...
            train_data_path = f"./data/inference_size_eta/train_n{n_samples_train}_seed_{seed}.csv"
            ...
            checkpoint_dir = f"./ckpt/inference_size_eta/conditional_n{n_samples_train}_seed_{seed}"
            ...
            for batch_idx, mc_batch_size in enumerate(mc_batch_sizes):
                ...
                # Fix Y1 and Y2 to observed (normalized) values; G2 only conditions on G0/Y1.
                do_vars = {}
                for i in range(10):
    ...
    overall_summary.to_csv(f"./results/inference_size_eta/summary{scaler_suffix}.csv")

    print(f"\nFinal results saved to ./results/inference_size_eta/:")
    print(f"- Detailed results: inference_size_eta_results{scaler_suffix}.csv")
