...
Train Cluster-2 style conditional diffusion models on shuffled Flow Cytometry
datasets, then run MCODEC inference for the edge p44/42 -> pakts473.
...
3. Train conditional diffusion with DAG:
      G0=[PIP3, PKA] -> G1=[p44/42]
      G0=[PIP3, PKA] -> G2=[pakts473]
4. Standardize observed variables and compute
      tau = MCODEC(z(pakts473), z(p44/42), z([PIP3, PKA]))
5. Sample synthetic pakts473 conditional on observed (PIP3, PKA),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train Cluster-2 style diffusion models on shuffled Flow Cytometry data "
            "and run standardized-MCODEC inference for p44/42 -> pakts473."
        )
    )
    parser.add_argument(
        "--n",
    ...
    print("=" * 80)
    print("Shuffled Flow Cytometry Inference (standardized stats): p44/42 -> pakts473")
    print("=" * 80)
    ...
    # Cluster-2 setup on 4 variables: [PIP3, PKA, p44/42, pakts473]
    ...
    for n_curr in n_values:
        results_path = args.results_dir / (
            f"p44_42_to_pakts473_shuffled_n{n_curr}_rep{args.n_rep}{scaler_suffix}.csv"
        )
        ...
        for iteration in range(args.start_iteration, args.start_iteration + args.n_rep):
            ...
            pip3_pka_center = pip3_pka.mean(axis=0, keepdims=True)
            pip3_pka_scale = pip3_pka.std(axis=0, ddof=0, keepdims=True)
            p44_42_center = p44_42.mean(axis=0, keepdims=True)
            p44_42_scale = p44_42.std(axis=0, ddof=0, keepdims=True)
            pakts473_center = pakts473_obs.mean(axis=0, keepdims=True)
            pakts473_scale = pakts473_obs.std(axis=0, ddof=0, keepdims=True)

            pip3_pka_z = standardize_with_reference(pip3_pka, pip3_pka_center, pip3_pka_scale)
            p44_42_z = standardize_with_reference(p44_42, p44_42_center, p44_42_scale)
            pakts473_obs_z = standardize_with_reference(pakts473_obs, pakts473_center, pakts473_scale)

            tau_observed = mcodec(pakts473_obs_z, p44_42_z, pip3_pka_z)
            print(f"tau_observed = {tau_observed:.6f}")

            print(f"Sampling synthetic pakts473 conditional on observed (PIP3, PKA), D={D}...")
            tau_samples = []
            ...
            for batch_idx, mc_batch_size in enumerate(mc_batch_sizes):
                ...
                for j in range(mc_batch_size):
                    ...
                    full_norm[:, 0:2] = data_norm[:, 0:2]
                    full_norm[:, 2] = data_norm[:, 2]
                    ...
                    tau_d = mcodec(pakts473_sampled_z, p44_42_z, pip3_pka_z)
