...
datasets, then run dependence-based edge inference for (PIP2, plcg) -> PKC.

Per iteration i:
1. Load flow_cytometry_inference/shuffled_data/Flow_Cytometry_i.csv
2. Take first n rows and keep columns: (PIP2, plcg, PKC)
3. Train conditional diffusion with DAG:
      G0=[PKC] (unconditional)
4. Standardize observed variables and compute
      tau = CODEC(z(PKC), z([PIP2, plcg]))
5. Sample synthetic PKC unconditionally from trained model,
   standardize synthetic PKC with observed PKC center/scale,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and run standardized-CODEC edge inference for (PIP2, plcg) -> PKC."
        )
    )
    parser.add_argument(
        "--n",
    ...
    parser.add_argument("--monte-carlo-size", type=int, default=100, help="Number of synthetic PKC samples (D).")
    ...
    print("=" * 80)
    print("Shuffled Flow Cytometry Inference (standardized stats): (PIP2, plcg) -> PKC")
    print("=" * 80)
    ...
    # Cluster-0 setup on 1 variable: [PKC]
    ...
    for n_curr in n_values:
        results_path = args.results_dir / (
            f"pip2_plcg_pkc_shuffled_n{n_curr}_rep{args.n_rep}{scaler_suffix}.csv"
        )
        ...
        for iteration in range(args.start_iteration, args.start_iteration + args.n_rep):
            ...
            if args.use_standard_scaler:
                ...
                pkc_norm = scaler.fit_transform(pkc_train)
            else:
            ...
            data_norm_t = torch.tensor(pkc_norm, dtype=torch.float32)
            ...
            pip2_plcg_center = pip2_plcg.mean(axis=0, keepdims=True)
            pip2_plcg_scale = pip2_plcg.std(axis=0, ddof=0, keepdims=True)
            pkc_center = pkc_obs.mean(keepdims=True)
            pkc_scale = pkc_obs.std(ddof=0, keepdims=True)

            pip2_plcg_z = standardize_with_reference(pip2_plcg, pip2_plcg_center, pip2_plcg_scale)
            pkc_obs_z = standardize_with_reference(pkc_obs, pkc_center, pkc_scale)

            tau_observed = codec(pkc_obs_z, Z=pip2_plcg_z)
            print(f"tau_observed = {tau_observed:.6f}")

            print(f"Sampling synthetic PKC unconditionally, D={D}...")
            tau_samples = []
            ...
            for batch_idx, mc_batch_size in enumerate(mc_batch_sizes):
                ...
                pkc_batch_norm = generate_conditional_samples(
                    models=models,
                    groups=groups,
                    A=A,
                    do_vars=None,
                ...
                pkc_batch_norm = pkc_batch_norm.squeeze(1).cpu().detach().numpy()
                pkc_batch = scaler.inverse_transform(pkc_batch_norm).reshape(mc_batch_size, n_curr)

                for j in range(mc_batch_size):
                    pkc_sampled = pkc_batch[j]
                    pkc_sampled_z = standardize_with_reference(pkc_sampled, pkc_center, pkc_scale)
                    tau_d = codec(pkc_sampled_z, Z=pip2_plcg_z)
